...
with open(path+TMPFILE,'rb') as f:
    [data_time,data_Y,data_X,data_X_smooth,data_treat,tfidf,data_location] = pickle.load(f)
...
data_Y_cf_all = []
# for treat_id in range(data_treat.shape[-1]-1):
for treat_id in range(TREAT+1):
...
with open(path+'cf_data_w{}_h{}_p{}.pkl'.format(WINDOW,HORIZON,PREWINDOW),'wb') as f:
    pickle.dump({'TIME':data_time,'Y':data_Y,'X':data_X,'X_sm':data_X_smooth,'C':data_treat,'CF_Y':data_Y_cf_all, 'CF_C':data_treat_cf,'location':data_location},f)
