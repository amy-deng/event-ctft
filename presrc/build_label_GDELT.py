...
file_list = glob.glob(mydir + "*{}*.json".format(DATASET))
...
for i,row in df.iterrows():
    full_loc = row['ActionGeo_Fullname'].split(', ')
    if len(full_loc) == 2:
        city.append('')
        district.append(full_loc[0])
    elif len(full_loc) == 3:
        city.append(full_loc[0])
        district.append(full_loc[1])
    else:
        city.append('')
        district.append('')
df['City']=city
...
if DATASET == 'NI':
    df = df.loc[df['District'].isin(['Abuja Federal Capital Territory'])]
    print('select locations for NI  #event =',len(df))

#TODO  how to get news??
