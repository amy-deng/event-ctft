...
try:
    ...
    print("Usage: DATASET (IND), WINDOW=10, HORIZON=1, PREDWINDOW=3")
    exit()
...
rawfile = "/home/sdeng/data/icews/events.sent.new.1991.201703.country/icews_events_{}.json".format(DATASET)

df = pd.read_json(rawfile,lines=True)
df['event_date'] = pd.to_datetime(df['Event Date'])
print('#event =',len(df))

if DATASET == 'IND':
    df = df.loc[df['City'].isin(['New Delhi','Delhi','Mumbai'])]
    print('select locations for IND  #event =',len(df))
elif DATASET =='RUS':
    df = df.loc[df['City'].isin(['Moscow Kremlin','Moscow','St. Petersburg'])]
...
#     print('select locations for GBR  #event =',len(df))
    ...
    df = df.loc[df['City'].isin(['Ankara','Istanbul'])]
    print('select locations for TUR  #event =',len(df))  
elif DATASET =='PAK':
    df = df.loc[df['City'].isin(['Islamabad','Kashmir','Karachi'])]
    print('select locations for TUR  #event =',len(df))
else:
    print('select other country')
    exit()
...
for location in locations:
    ...
    df_location = df.loc[df['City'] == location]
    ...
    while start <= end:
        start_date_str = start.strftime("%Y-%m-%d")
        df_day = df_location.loc[df_location['event_date'] == start_date_str]
        df_count = df_day['RootEventCode'].value_counts().rename_axis('unique_values').reset_index(name='counts')
    ...
    for i in range(WINDOW,len(date_ids),PREDWINDOW): # no overlap of pre_window
        ...
        # if curr at leat three days no news, do not predict
        if len(curr)-np.count_nonzero(curr) >= 3:
            continue
        data_X.append(curr)
        ...
        df_window = df_location.loc[df_location['event_date'].isin(date_list)]['Event Sentence']
...
vectorizer = TfidfVectorizer(tokenizer=(lambda x:x), lowercase=True, stop_words=stop_words,min_df=10,max_df=0.99)#token_pattern=r'(?u)\b\w*[a-zA-Z]\w*\b') # stopwords='english' u'(?u)\b\w*[a-zA-Z]\w*\b
...
with open(path+'basic_data_w{}_h{}_p{}.pkl'.format(WINDOW,HORIZON,PREDWINDOW),'wb') as f:
    pickle.dump([data_time,data_Y,data_X,data_X_smooth,data_treat,tfidf,data_location],f)
