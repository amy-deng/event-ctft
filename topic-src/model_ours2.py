...
class ours_causal51(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        ...
        for _ in range(n_layers):
            if self.with_rdm:
                self.gcs.append(causal_message_passing_rdm51(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
            else:
                print('error')
                exit()
                pass
