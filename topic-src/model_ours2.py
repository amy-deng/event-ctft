...
# with time added a weight term in reduce
class causal_message_passing_rdm2(nn.Module):
    def __init__(self, in_dim, out_dim, ntypes, etypes, n_heads, dropout = 0.5, use_norm = False, device=torch.device("cpu")):
        super().__init__()
        self.in_dim        = in_dim
        self.out_dim       = out_dim
...
# also consider the weights, extra embedding
class causal_message_passing_rdm3(nn.Module):
    def __init__(self, in_dim, out_dim, ntypes, etypes, n_heads, dropout = 0.5, use_norm = False, device=torch.device("cpu")):
        super().__init__()
        self.in_dim        = in_dim
        self.out_dim       = out_dim
        ...
        # self.relation_att = nn.ParameterDict()
        self.relation_msg = nn.ParameterDict()
        self.graph_topo_linear = nn.ModuleDict()
        for etype in etypes:
            # self.relation_pri[etype] = nn.Parameter(torch.ones(self.n_heads))
            # self.relation_att[etype] = nn.Parameter(torch.Tensor(n_heads, self.d_k, self.d_k))
            self.relation_msg[etype] = nn.Parameter(torch.Tensor(n_heads, self.d_k, self.d_k))
            self.graph_topo_linear[etype] = nn.Linear(in_dim, out_dim)
    ...
    def edge_attention(self, etype):
        def msg_func(edges):
            ...
            cau_val = None
            # graph_topo_linear
            # print(edges.src['th'].shape,'src',edges.data['weight'].shape,'weight')
            topo_msg = edges.src['th'] * edges.data['weight'].unsqueeze(-1)
            if etype in ['tw','tt','td']:
                # if src is causal node
                # causal edges
                # src node 
                ...
                cau_val   = torch.bmm(edges.src['v'].transpose(1,0) + self.time_emb, relation_msg_cau).transpose(1,0)
                # print(cau_key.shape,'cau_key',cau_att.shape,'cau_att',cau_val.shape,'cau_val')
                return {'a': att, 'v': val, 'ca':cau_att,'cv':cau_val, 'tm':topo_msg}
            return {'a': att, 'v': val, 'tm':topo_msg}
        return msg_func
    ...
    def reduce_func(self, etype):
        def reduce(nodes):
            ...
            if 'ca' in nodes.mailbox:
                ...
                h = beta * h + (1-beta) * cau_h
                # h += cau_h
            th = torch.sum(nodes.mailbox['tm'], dim=1)
            return {'t': h.view(-1, self.out_dim) + th}
        return reduce
    ...
    def forward(self, G, inp_key, out_key):
        ...
        for srctype, etype, dsttype in G.canonical_etypes:
            ...
            v_linear = self.v_linears[srctype] 
            q_linear = self.q_linears[dsttype]
            graph_topo_linear = self.graph_topo_linear[etype]
            G.nodes[srctype].data['k'] = k_linear(G.nodes[srctype].data[inp_key]).view(-1, self.n_heads, self.d_k)
            G.nodes[srctype].data['v'] = v_linear(G.nodes[srctype].data[inp_key]).view(-1, self.n_heads, self.d_k)
            G.nodes[dsttype].data['q'] = q_linear(G.nodes[dsttype].data[inp_key]).view(-1, self.n_heads, self.d_k)
            G.nodes[srctype].data['th'] = graph_topo_linear(G.nodes[srctype].data[inp_key]).view(-1, self.out_dim)
...
# add prior-based, inside attention score
...
class causal_message_passing_rdm5(nn.Module):
    def __init__(self, in_dim, out_dim, ntypes, etypes, n_heads, dropout = 0.5, use_norm = False, device=torch.device("cpu")):
        super().__init__()
        self.in_dim        = in_dim
        self.out_dim       = out_dim
    ...
    def edge_attention(self, etype, inp_key):
        def msg_func(edges):
            ...
            relation_msg = self.relation_msg[etype] 
            # print(att0,'att0')
            # att1 = (edges.dst['q'] * edges.src['k'] ).sum(dim=-1)/ self.sqrt_dk
            # print(att1,'att1')
...
class causal_message_passing_rdm51(nn.Module):
    def __init__(self, in_dim, out_dim, ntypes, etypes, n_heads, dropout = 0.5, use_norm = False, device=torch.device("cpu")):
        super().__init__()
        self.in_dim        = in_dim
        self.out_dim       = out_dim
...
# no shared key and value
class causal_message_passing_rdm_content_prior(nn.Module):
    def __init__(self, in_dim, out_dim, ntypes, etypes, n_heads, dropout = 0.5, use_norm = False, device=torch.device("cpu")):
        super().__init__()
        self.in_dim        = in_dim
        self.out_dim       = out_dim
...
# MAIN ablation no prior...
class causal_message_passing_rdm_content(nn.Module):
    def __init__(self, in_dim, out_dim, ntypes, etypes, n_heads, dropout = 0.5, use_norm = False, device=torch.device("cpu")):
        super().__init__()
        self.in_dim        = in_dim
        self.out_dim       = out_dim
...
# no shared key and value, (correlation,causal)W
class causal_message_passing_rdm_content_prior3(nn.Module):
    def __init__(self, in_dim, out_dim, ntypes, etypes, n_heads, dropout = 0.5, use_norm = False, device=torch.device("cpu")):
        super().__init__()
        self.in_dim        = in_dim
        self.out_dim       = out_dim
        ...
        # self.comb_pri = nn.ParameterDict()
        self.relation_msg_cau = nn.ParameterDict()
        self.comb_mx1 = nn.ParameterDict()
        self.comb_mx2 = nn.ParameterDict()

        for etype in ['tw','tt','td']:
            self.relation_msg_cau[etype] = nn.Parameter(torch.Tensor(n_heads, self.d_k, self.d_k))
            # self.comb_pri[etype] = nn.Parameter(torch.ones(1))
            self.comb_mx1[etype] = nn.Parameter(torch.Tensor(out_dim*2,16))
            self.comb_mx2[etype] = nn.Parameter(torch.Tensor(16, out_dim))
        self.cau_filter = nn.Parameter(torch.Tensor(3, n_heads, self.d_k, self.d_k))
        self.drop           = nn.Dropout(dropout)
        self.init_weights()
    ...
    def reduce_func(self,etype):
        def reduce(nodes):
            ...
            if 'ca' in nodes.mailbox:
                ...
                # beta = torch.sigmoid(self.comb_pri[etype])
                # print(beta,'beta',self.comb_pri[etype])
                # print(h.shape,cau_h.shape)
                # h = beta * h + (1-beta) * cau_h
                # h += cau_h
                # print(h.shape,'1')
                h_cau_h = torch.cat((h, cau_h.view(-1, self.out_dim)),dim=-1)
                # print(h_cau_h.shape,'h_cau_h')
                weight = torch.mm(self.comb_mx1[etype], self.comb_mx2[etype])
                # print(weight.shape,'weight')
                h = torch.mm(h_cau_h,weight)
            return {'t': h}
        return reduce
...
# no shared key and value ONLY prior
class message_passing_prior(nn.Module):
    def __init__(self, in_dim, out_dim, ntypes, etypes, n_heads, dropout = 0.5, use_norm = False, device=torch.device("cpu")):
        super().__init__()
        self.in_dim        = in_dim
        self.out_dim       = out_dim
    ...
    def edge_attention(self, etype, inp_key):
        def msg_func(edges):
            ...
            query_prior =  (edges.data['weight'] * relation_att).permute(2,0,1).contiguous()
            relation_msg = self.relation_msg[etype] 
            att   = ((query_prior * edges.src['k'] ).sum(dim=-1) / self.sqrt_dk )#+ att0
            val   = torch.bmm(edges.src['v'].transpose(1,0), relation_msg).transpose(1,0)
            return {'a': att, 'v': val}
        return msg_func
...
class ours_causal2(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        ...
        for _ in range(n_layers):
            if self.with_rdm:
                self.gcs.append(causal_message_passing_rdm2(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
            else:
                self.gcs.append(causal_message_passing(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
        self.out_layer = nn.Sequential(
                # nn.Linear(n_hid*3, n_hid),
...
class ours_causal4(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        ...
        for _ in range(n_layers):
            if self.with_rdm:
                self.gcs.append(causal_message_passing_rdm4(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
            else:
                self.gcs.append(causal_message_passing(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
        self.out_layer = nn.Sequential(
                # nn.Linear(n_hid*3, n_hid),
...
class ours_causal5(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        ...
        for _ in range(n_layers):
            if self.with_rdm:
                self.gcs.append(causal_message_passing_rdm5(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
            else:
                print('error')
                exit()
                pass
...
# only use 3/7/14 causal nodes

class ours_causal7_one(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False, one_causal=0):
        super().__init__()
        self.n_inp = n_inp
        self.n_hid = n_hid
        self.n_layers = n_layers
    ...
    def forward(self, g_list, y_data): 
        ...
        bg.nodes['word'].data['h0'] = word_emb
        # topic_ids = bg.nodes['topic'].data['id'].long()
        effect = bg.nodes['topic'].data['effect'][:,self.one_causal]#.to_dense()
        effect = (effect >0)*1. + (effect < 0)*(-1.)
        # effect = effect.sum(-1)
        effect = ((effect > 0)*1.) + ((effect < 0)*2.)
...
# rdm causal nodes
class ours_causal7_rdm(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        super().__init__()
        self.n_inp = n_inp
        self.n_hid = n_hid
    ...
    def forward(self, g_list, y_data): 
        ...
        bg.nodes['word'].data['h0'] = word_emb
        # topic_ids = bg.nodes['topic'].data['id'].long()
        # print(bg.nodes['topic'].data['effect'].shape, bg.nodes['topic'].data['id'].shape,'~~~~')
        # effect = bg.nodes['topic'].data['effect']#.to_dense()
        # effect = (effect >0)*1. + (effect < 0)*(-1.)
        # effect = effect.sum(-1)
        # effect = ((effect > 0)*1.) + ((effect < 0)*2.)
        # # count
        # import collections
        # r  = collections.Counter(effect.view(-1).numpy())
        # print(r,len(bg.nodes['topic'].data['id']))
...
# no dw,dt no prior
class ours_causal7_nop(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        super().__init__()
        self.n_inp = n_inp
        self.n_hid = n_hid
        ...
        for _ in range(n_layers):
            if self.with_rdm:
                self.gcs.append(causal_message_passing_rdm_content(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
            else:
                print('error')
                exit()
                pass
...
class ours_causal73(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        ...
        for _ in range(n_layers):
            if self.with_rdm:
                self.gcs.append(causal_message_passing_rdm_content_prior3(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
            else:
                print('error')
                exit()
                pass
...
class ours_temp7(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        ...
        self.gcs = nn.ModuleList()
        for _ in range(n_layers):
            self.gcs.append(message_passing_content_prior2(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
        self.out_layer = nn.Sequential(
                # nn.Linear(n_hid*3, n_hid),
                # nn.BatchNorm1d(n_hid),
                nn.Linear(n_hid*3, 1) 
...
class ours_temp72(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, n_heads, activation, device, seq_len, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True, with_rdm=False):
        ...
        self.gcs = nn.ModuleList()
        for _ in range(n_layers):
            self.gcs.append(message_passing_prior(n_hid, n_hid, ntypes, etypes, n_heads, use_norm = use_norm, device=self.device))
        self.out_layer = nn.Sequential(
                # nn.Linear(n_hid*3, n_hid),
                # nn.BatchNorm1d(n_hid),
                nn.Linear(n_hid*3, 1) 
