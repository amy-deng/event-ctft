...
class Hetero(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, activation, device, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True):
        ...
        for _ in range(n_layers):
            self.hetero_layers.append(HeteroConvLayer(n_hid, n_hid, activation, dropout, ['ww','wt','tt','wd','td'],['word','topic','doc']))
...
class HeteroAll(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, activation, device, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True):
        super(HeteroAll, self).__init__()
        self.n_inp = n_inp
        self.n_hid = n_hid
        self.n_layers = n_layers
        self.vocab_size = vocab_size
