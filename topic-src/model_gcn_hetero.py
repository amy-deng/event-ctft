...
class HeteroConvLayer(nn.Module):
    def __init__(self, in_feats, out_feats, activation, dropout, etypes):
        super().__init__()
        self.etypes = etypes
        self.weight = nn.ModuleDict()
        for etype in etypes:
...
class GCNHet(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, activation, device, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True):
        ...
        for _ in range(n_layers):
            self.hetero_layers.append(HeteroConvLayer(n_hid, n_hid, activation, dropout, ['wt','wd','td']))
...
'''concate three types of embedding for final prediction'''
class GCNHetAll(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, activation, device, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True):
        super(GCNHetAll, self).__init__()
        self.n_inp = n_inp
        self.n_hid = n_hid
        self.n_layers = n_layers
        self.vocab_size = vocab_size
...
class GCNHetAll2(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, activation, device, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True):
        super(GCNHetAll2, self).__init__()
        self.n_inp = n_inp
        self.n_hid = n_hid
        self.n_layers = n_layers
        self.vocab_size = vocab_size
    ...
    def forward(self, g_list, y_data): 
        ...
        for i in range(len(self.gcn_word_layers)):
            ...
            feat_dict = {'word':word_emb, 'topic':topic_emb}
            feat_dict = self.hetero_layers[i](bg, feat_dict)
...
class GCNHetAll3(nn.Module):
    def __init__(self, n_inp, n_hid, n_layers, activation, device, num_topic=50, vocab_size=15000, dropout=0.5, pool='max', use_norm = True):
        ...
        self.hetero_layers = nn.ModuleList()
        for _ in range(n_layers):
            self.hetero_layers.append(HeteroConvLayer(n_hid, n_hid, activation, dropout, ['wt','tt','wd','td']))
        self.out_layer = nn.Sequential(
                nn.Linear(n_hid*3, n_hid),
                nn.BatchNorm1d(n_hid),
                nn.Linear(n_hid, 1) 
