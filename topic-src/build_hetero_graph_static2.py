...
if vocab_size > 0:
    outf = dataset_path + '/hetero_static_{}_{}_{}_docpmi.pkl'.format(start_date,stop_date,vocab_size)
else:
    outf = dataset_path + '/hetero_static_{}_{}_docpmi.pkl'.format(start_date,stop_date)
print(outf)
...
def word_word_pmi_doc(tokens_list, sample_words):
    '''
...
# topic_vocab = []
# for topic_id in range(num_topics):
#     top_word_weights = loaded_lda.get_topic_terms(topic_id,200)
#     topic_vocab += [loaded_dict[v[0]] for v in top_word_weights]
# topic_vocab = list(set(topic_vocab))
...
for i,row in df.iterrows():
    ...
    # word---word
    word_i, word_j, weight = word_word_pmi_doc(tokens_list, sample_words) 
    ...
    print('i={} \t {}  \t {} day_has_data \t cur_end_date:{} {}'.format(i,date,city,1,time.ctime()))
    # if len(all_g_list) >= 3:
        # break
