...
class DataLoaderLinearCausal(object):
    def __init__(self, args):
        ...
        sum_event = self.data.sum(-1) + 1e-6
        print(self.y.shape,'y',sum_event.shape)
        self.y = self.y/sum_event
        print("data {} \t y {}".format(self.data.shape, self.y.shape))
        print(self.y.mean(1))
        with open('{}/{}/nei_weight_emb.pkl'.format(args.data_path, args.dataset), 'rb') as f:
            self.confounder = pkl.load(f)
    ...
    def _split(self, train, val, test):
        ...
        n_train = len(self.train[0])
        n_val = len(self.val[0])
        n_test = len(self.test[0])
        self.train.append(torch.from_numpy(self.confounder[:n_train]))
        self.val.append(torch.from_numpy(self.confounder[n_train:n_train+n_val]))
        self.test.append(torch.from_numpy(self.confounder[-n_test:]))
    ...
    def _batchify(self, idx_set):  
        ...
        # for i in range(0,n):
            ...
            tmp_x = self.data[:,start:end, :].sum(1)
            X[i,:,:] = torch.from_numpy(tmp_x)
...
def eval_regression(y_true, y_pred):
    r = {}
    r['mae'] = metrics.mean_absolute_error(y_true, y_pred)
    r['mse'] = metrics.mean_squared_error(y_true, y_pred)
    return r