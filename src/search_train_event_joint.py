...
def run(cfg_file, num_runs, model, dataset, window, horizon, pred_window, treat_idx, gpu):
    ...
    append_config = " -m {} -d {} --window {} --horizon {} --pred_window {} --gpu {} --treat_idx {} ".format(model,dataset,window,horizon,pred_window,gpu,treat_idx)
    # print(configs)
    ...
    used_cfg_path = '{}/{}/{}_w{}h{}p{}_treat{}'.format(outdir,dataset,model,window,horizon,pred_window,treat_idx)
    ...
    for i in range(num_runs):
        ...
        flags += append_config
        print('python train_event_joint.py {}'.format(flags))
        call('python train_event_joint.py {}'.format(flags), shell=True)
...
if __name__ == "__main__":
    print(len(sys.argv))
    if len(sys.argv) < 10:
        print ('Usage: python param_search.py <config file> <num runs> <model name> <dataset> <window> <horizon> <pred window> <treat idx> <gpu>')
    else:
        # print(sys.argv)
        run(sys.argv[1], int(sys.argv[2]), sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], int(sys.argv[9]))