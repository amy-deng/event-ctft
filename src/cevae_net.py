...
class p_y_zt(nn.Module):
    ...
    def forward(self, z, t):
        ...
        if self.bi_outcome:
            ...
            # print(t.shape,'t',mu_t0.shape,'mu_t0')
...
class q_z_tyx(nn.Module):
    ...
    def forward(self, xy, t):
        ...
        # ttt = self.input.weight.data
        # print(ttt.shape,ttt[0][:5],'ttt')
        # print('first linear z_infer')
        # print(x)
        for i in range(self.nh):
            x = F.elu(self.hidden[i](x))
        # print(x.shape,x,'xxxxx')
        mu_t0 = self.mu_t0(x)
        mu_t1 = self.mu_t1(x)
        ...
        # print(mu_t0.shape,mu_t0,'mu_t0',sigma_t0.shape,sigma_t0,'sigma_t0')
        # print(mean.shape,mean,'mean',std.shape,'std',std)
        z = normal.Normal(mean, std)
        # z = bernoulli.Bernoulli((1-t)*mu_t0 + t * mu_t1)
        return z
...
class CEVAE(nn.Module):
    def __init__(self, x_dim, h_dim, z_dim, binfeats, contfeats, device, bi_outcome=True):
        super().__init__()
        # binfeats contfeats 
        # self.insnorm = nn.InstanceNorm1d(x_dim)
        # self.mlp = nn.Linear(x_dim, h_dim)
        self.device = device
    ...
    def forward(self, x_train, y_train, t_train):
        # x_train = self.mlp(self.insnorm(x_train.unsqueeze(0))).squeeze(0)
        # x_train = self.insnorm(x_train.unsqueeze(0)).squeeze(0)
        ...
        while True:
            if (z_infer_sample != z_infer_sample).all(): #NaN
                ...
                # print(z_infer_sample,'z_infer_sample')
                # raise ValueError('z_infer.sample() forward contains NaN in sampled z')
                break
...
def init_qz(qz, pz, y,t,x, device):
    ...
    for i in range(50):
        ...
        # print(batch,'batch',type(x),x.dtype,x.shape,type(t),t.shape,t.dtype,type(y),y.shape,y.dtype)
        # batch = torch.tensor(batch)
        # print(x[batch])
        
        if str(device) == 'cpu':
            x_train = torch.FloatTensor(x[batch])
            y_train = torch.FloatTensor(y[batch])
            t_train =  torch.FloatTensor(t[batch])
            # x_train, y_train, t_train = torch.FloatTensor(x[batch]), torch.FloatTensor(y[batch]), torch.FloatTensor(t[batch])
